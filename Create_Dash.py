...
if __name__ == '__main__':
  ...
  if choice=='DataIntroduction':
    ...
    ax2=plt.subplot(132)
    a2 = sns.barplot(data=sum_by_vendor, x='Vendor_code', y='revenue', estimator=sum, ci=0, ax=ax2, palette='Blues_r')
    ax2.set_xticklabels(sum_by_vendor['Vendor_code'].unique(),rotation=90, fontsize=18)
    ax2.set_yticks(a2.get_yticks())
    ax2.set_yticklabels(labels=[str(i/1000000)+'M' for i in a2.get_yticks()], fontsize=18)
    ...
    ax3=plt.subplot(133)
    a3=sns.barplot(data=sum_by_vendor, x='Vendor_code', y='count_orders', estimator=sum, ci=0, ax=ax3, palette='Blues_r')
    ax3.set_xticklabels(sum_by_vendor['Vendor_code'].unique(),rotation=90, fontsize=18)
    ax3.set_yticks(a3.get_yticks())
    ax3.set_yticklabels(labels=[str(i/1000000)+'M' for i in a3.get_yticks()], fontsize=14)
    ...
    a41=sns.barplot(data=sum_by_status, x='Trạng Thái', y='revenue', estimator=sum, ci=0, ax=ax41, palette='Blues_r')
    ax41.set_xticklabels(sum_by_status['Trạng Thái'].unique(),rotation=90, fontsize=20)
    ax41.set_yticks(a41.get_yticks())
    ax41.set_yticklabels(labels=[str(i/1000000)+'M' for i in a41.get_yticks()], fontsize=20)
    ...
    a42=sns.barplot(data=sum_by_status, x='Trạng Thái', y='count_orders', estimator=sum, ci=0, ax=ax42, palette='Blues_r')
    ...
    f3=sns.lineplot(data=df.sort_values(by='month'), x='month', y='Actual', estimator=sum, marker='o', color='orange', hue='Vendor_code')
    plt.xticks(df['month'].unique(), fontsize=18)
    plt.yticks(f3.get_yticks(),[str(i/1000000)+'M' for i in f3.get_yticks()], fontsize=18)
    sns.despine(top=True, bottom=True, right=True, left=True)
    ...
    f4=sns.barplot(data=sum_by_vendor.groupby('tinh').sum().reset_index()[['tinh', 'revenue']].sort_values(by='revenue',ascending=False), x='tinh', y='revenue', palette='Blues_r', estimator=sum)
    plt.xticks(rotation=90, fontsize=18)
    plt.yticks(f4.get_yticks(),[str(i/1000000)+'M' for i in f4.get_yticks()], fontsize=20)
